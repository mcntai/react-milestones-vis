...
function time() {
  return calendar(timeYear, timeMonth, sunday, timeDay, timeHour, timeMinute, utcSecond, utcMillisecond, timeFormat$1).domain([new Date(2000, 0, 1), new Date(2000, 0, 2)]);
}
        ...
        if (
          ...
          if (optimizerRuns > 0 && orientation === 'horizontal') {
            ...
            nodes.forEach((overlapCheckNode, overlapCheckIndex) => {
              const overlapCheckItem = selectAll(overlapCheckNode)
                .data()[0];
              ...
              const overlapCheckDomElement = selectAll(
  ...
  function render(data) {
    ...
    timelineMerge.each((d, i, node) => {
      ...
      const maxAboveHeight = max(
        select(node[i]).selectAll('* .' + cssAboveClass + '-' + orientation)
          ._groups[0],
        (d) => d.offsetHeight
      );
...
var isPartialMapping = function (arg) {
    return typeof arg === 'object' &&
        arg !== null &&
        Object.keys(arg).every(function (d) {
            return milestonesMappingKeys.includes(d);
        });
};
var milestonesAggregateBy = [
    'second',
    'minute',
    'hour',
    'day',
...
var isAggregateBy = function (arg) {
    return typeof arg === 'string' &&
        milestonesAggregateBy.includes(arg);
};
...
var isRange = function (arg) {
    return Array.isArray(arg) &&
        arg.length === 2 &&
        arg.every(function (d) { return typeof d === 'number'; });
};
...
var Milestones = function (props) {
    var milestonesDivEl = react.useRef(null);
    var _a = react.useState(), vis = _a[0], setVis = _a[1];
    react.useEffect(function () {
        setVis(milestones(milestonesDivEl.current));
    }, []);
    react.useEffect(function () {
        if (vis) {
            var _a = __assign(__assign({}, getDefaults()), props), aggregateBy = _a.aggregateBy, mapping = _a.mapping, optimize = _a.optimize, autoResize = _a.autoResize, orientation_1 = _a.orientation, distribution = _a.distribution, parseTime = _a.parseTime, labelFormat = _a.labelFormat, useLabels = _a.useLabels, range = _a.range, onEventClick = _a.onEventClick, onEventMouseLeave = _a.onEventMouseLeave, onEventMouseOver = _a.onEventMouseOver, data = _a.data;
    ...
    return jsxRuntime.jsx("div", { ref: milestonesDivEl }, void 0);
};
